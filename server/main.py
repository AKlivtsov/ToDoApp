...
def api_user_login(item: LoginUser):
...
def api_create_task(
    ...
    return {"id": id_, "title": title, "description": desc}
...
def api_update_task(
    ...
    if check:
        return JSONResponse(
            status_code=403,
            content={"status": "error", "message": "Вы не являетесь автором задачи."},
        )
